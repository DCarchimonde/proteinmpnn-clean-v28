...
PYTHON_BIN="${V11_PYTHON:-python}"
OUTPUT_ROOT="${V11_OUTPUT_ROOT:-$REPO_ROOT/paper_clean_v28_outputs/cyclic_native_v11_1700_monomer}"
PLAN="${V11_PLAN:-$REPO_ROOT/paper_clean_v28/serine_qc_retrain/target_plan_v11_cyclic_native_rmsd_priority_1700.json}"
PARENT_MODEL="${V11_PARENT_MODEL:-$REPO_ROOT/frankenstein_v28.pt}"
TRAIN_JSONL="${V11_TRAIN_JSONL:-$REPO_ROOT/v9_inputs/train_serine_provenance_corrected.jsonl}"
TEST_JSONL="${V11_TEST_JSONL:-$REPO_ROOT/v9_inputs/test_serine_provenance_corrected.jsonl}"
NATIVE_JSONL="${V11_NATIVE_JSONL:-$REPO_ROOT/17_complexes_native.jsonl}"
BEST_CSV="${V11_BEST_CSV:-$REPO_ROOT/paper_clean_v28_outputs/generated_fasta_clean_auto_single/best_designs.csv}"
HISTORICAL_CSV="${V11_HISTORICAL_CSV:-$REPO_ROOT/paper_clean_v28_outputs/generated_fasta_clean_auto_single/all_designs.csv}"
PRIOR_CSV="${V11_PRIOR_CSV:-$REPO_ROOT/v9_inputs/methylated_new_candidates.csv}"
RMSD_DEVELOPMENT_CSV="${V11_RMSD_DEVELOPMENT_CSV:-$REPO_ROOT/v10_inputs/six_non3av_t05_joint_rmsd_476.csv}"
ORIGINAL_MONOMER_CORRECTED_CSV="${V11_ORIGINAL_MONOMER_CSV:-$REPO_ROOT/v10_inputs/monomer_corrected_1505_original_v28.csv}"
POSITION_CONCENTRATION_POLICY="${V11_POSITION_POLICY:-$REPO_ROOT/v10_inputs/evidence_aware_position_concentration_policy.json}"
PANDAS_SPEC="${V11_PANDAS_SPEC:-pandas==2.2.3}"
...
V11_FINAL_DIR="$OUTPUT_ROOT/final_v10_handoff"
...
TRANSFER_ARCHIVE="$OUTPUT_ROOT/v11_autodl_to_windows_handoff.tar.gz"
LOG="$OUTPUT_ROOT/v11_1700_monomer_pipeline.log"

export OMP_NUM_THREADS="${V11_OMP_THREADS:-16}"
export MKL_NUM_THREADS="${V11_MKL_THREADS:-16}"
export OPENBLAS_NUM_THREADS="${V11_OPENBLAS_NUM_THREADS:-16}"
export NUMEXPR_NUM_THREADS="${V11_NUMEXPR_NUM_THREADS:-16}"
...
verify_frozen_inputs() {
  "$PYTHON_BIN" - \
    ...
    "$PLAN" "618c7d68e4dd171ddd558ab9fceb58b9fc64769f648cc49134b38091aaf4b938" \
...
if failures:
    raise SystemExit("ERROR: frozen V11 input SHA-256 mismatch:\n" + "\n".join(failures))
print("Frozen V11 input SHA-256 contract: PASS")
PY
}
...
ensure_v10_regression_dependencies() {
  ...
  echo "V11 dependency bootstrap: pandas is missing; installing $PANDAS_SPEC"
  local index_url
  for index_url in \
    "${V11_PANDAS_INDEX_URL:-https://pypi.org/simple}" \
    "https://pypi.tuna.tsinghua.edu.cn/simple" \
    "https://mirrors.aliyun.com/pypi/simple"; do
    echo "V11 dependency bootstrap: trying HTTPS index $index_url"
    if "$PYTHON_BIN" -m pip install \
      ...
      "$PANDAS_SPEC"; then
      if "$PYTHON_BIN" -c 'import pandas' >/dev/null 2>&1; then
        echo "V11 dependency bootstrap: pandas import PASS"
        return
      fi
    fi
  done
  ...
  echo "WARNING: the AutoDL V11 training/generation/monomer-sequence runtime does not import pandas." >&2
}
...
v10_final_handoff_passes() {
  manifest_passes "$V11_FINAL_MANIFEST" \
    "$RMSD_REPLAYER" "$FINAL_MANIFEST" "$SELECTION_MANIFEST" "$RMSD_MANIFEST" "$RMSD_MODELS" \
    "$V11_FINAL_DIR/1700_详细审计.csv" \
    "$V11_FINAL_DIR/1700_给尚哥_极简.csv" \
    "$V11_FINAL_DIR/1700_给尚哥_结构输入.fasta" \
    "$V11_FINAL_DIR/v10_rmsd_priority_replay.csv" || return 1
  "$PYTHON_BIN" - \
    "$V11_FINAL_DIR/1700_详细审计.csv" \
    "$V11_FINAL_DIR/v10_rmsd_priority_replay.csv" <<'PY'
import collections
import csv
import pathlib
import sys
...
require_empty_stage() {
  ...
  if [[ -d "$stage_dir" ]] && [[ -n "$(find "$stage_dir" -mindepth 1 -maxdepth 1 -print -quit)" ]]; then
    ...
    echo "Use a new V11_OUTPUT_ROOT; this launcher never deletes scientific evidence." >&2
    exit 1
  fi
}

echo "===== V11 CYCLIC-NATIVE + RMSD-PRIORITY 17 x 100 + MONOMER PIPELINE ====="
...
if not torch.cuda.is_available():
    raise SystemExit("ERROR: CUDA PyTorch is required for V11 retraining/replay")
print(json.dumps({
...
if [[ "${V11_SKIP_TESTS:-0}" != "1" ]]; then
  echo "[1/10] Full code regression"
  (cd "$REPO_ROOT" && "$PYTHON_BIN" -m unittest discover -s tests)
else
  echo "[1/10] Full code regression skipped only because V11_SKIP_TESTS=1"
fi
...
if [[ -f "$MODEL" ]] && manifest_passes "$MODEL_MANIFEST" \
  ...
  "$PYTHON_BIN" "$TRAINER" \
    ...
    --out-dir "$MODEL_DIR" \
    --epochs "${V11_EPOCHS:-80}" \
    --batch-size "${V11_TRAIN_BATCH_SIZE:-16}" \
    ...
    --positional-learning-rate "${V11_POSITIONAL_LEARNING_RATE:-2e-5}" \
    --base-sequence-loss-weight "${V11_BASE_SEQUENCE_LOSS_WEIGHT:-0.25}" \
    --positional-anchor-weight "${V11_POSITIONAL_ANCHOR_WEIGHT:-0.01}" \
    --maximum-equivariance-span "${V11_MAXIMUM_EQUIVARIANCE_SPAN:-1e-5}" \
    --maximum-base-ce-increase "${V11_MAXIMUM_BASE_CE_INCREASE:-0.05}" \
    --maximum-base-accuracy-drop "${V11_MAXIMUM_BASE_ACCURACY_DROP:-0.02}"
fi
...
if [[ -f "$AUDIT_JSON" ]] && manifest_passes "$AUDIT_JSON" \
  ...
  "$PYTHON_BIN" "$AUDITOR" \
    ...
    --plan "$PLAN" \
    --out-dir "$AUDIT_DIR" \
    --batch-size "${V11_AUDIT_BATCH_SIZE:-8}" \
    --temperature 0.5 \
    --threshold 0.6 \
    --required-expert-protocol "canonical_clean_v28_all_expert_heads_cyclic_native_relative_positions_v11" \
    --device cuda
fi
...
monomer_handoff_passes || {
  echo "ERROR: V11 monomer audit did not pass" >&2
  exit 1
}
...
if [[ ! -f "$GENERATION_MANIFEST" ]]; then
  ...
  "$PYTHON_BIN" "$GENERATOR" \
    ...
    --batch_size "${V11_GENERATION_BATCH_SIZE:-16}" \
    --device cuda \
...
if [[ -f "$BASE_MANIFEST" ]] && manifest_passes "$BASE_MANIFEST" \
  ...
  "$PYTHON_BIN" "$BASE_SCORER" \
    ...
    --plan "$PLAN" \
    --out-dir "$BASE_DIR" \
    --batch-size "${V11_BASE_BATCH_SIZE:-32}" \
    --device cuda
fi
...
if [[ -f "$RMSD_MANIFEST" ]] && manifest_passes "$RMSD_MANIFEST" \
  ...
  require_empty_stage "$RMSD_DIR" "V11 RMSD-priority ranker stage"
...
if [[ -f "$SELECTION_MANIFEST" ]] && manifest_passes "$SELECTION_MANIFEST" \
  ...
  require_empty_stage "$SELECTION_DIR" "V11 17 x 100 selection stage"
...
if [[ -f "$FINAL_MANIFEST" ]] && final_handoff_passes; then
  ...
  require_empty_stage "$FINAL_DIR" "V11 lower-layer independent replay stage"
...
if [[ -f "$V11_FINAL_MANIFEST" ]] && v10_final_handoff_passes; then
  echo "[10/10] Reusing independent RMSD-priority replay and V11 final package"
else
  require_empty_stage "$V11_FINAL_DIR" "V11 final RMSD-priority replay stage"
  ...
  "$PYTHON_BIN" "$RMSD_REPLAYER" \
    ...
    --out-dir "$V11_FINAL_DIR"
fi
v10_final_handoff_passes || {
  echo "ERROR: V11 final handoff did not pass" >&2
  exit 1
}
if [[ -f "$REPORT_MANIFEST" ]] && manifest_passes "$REPORT_MANIFEST" \
  ...
  "$RMSD_MANIFEST" "$V11_FINAL_MANIFEST" "$MONOMER_MANIFEST" \
  ...
  require_empty_stage "$REPORT_DIR" "V11 pre-structure report stage"
  "$PYTHON_BIN" "$REPORT_BUILDER" \
    ...
    --ranker-manifest "$RMSD_MANIFEST" \
    --v10-final-manifest "$V11_FINAL_MANIFEST" \
...
echo "===== V11 ALL PRE-STRUCTURE GATES PASSED ====="
echo "IMPORTANT: RMSD improvement is predicted, not proven until returned structures are audited."
echo "Detailed: $V11_FINAL_DIR/1700_详细审计.csv"
echo "Shang-ge concise: $V11_FINAL_DIR/1700_给尚哥_极简.csv"
echo "Shang-ge FASTA: $V11_FINAL_DIR/1700_给尚哥_结构输入.fasta"
echo "V11 final manifest: $V11_FINAL_MANIFEST"
