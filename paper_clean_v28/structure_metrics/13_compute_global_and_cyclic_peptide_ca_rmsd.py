...
def residue_to_design_letter(record_name: str, resname: str) -> str:
    """Decode ATOM as natural and HETATM as N-methyl, preserving case."""
    if record_name == "HETATM":
        return NMETHYL_RESIDUE_MAP.get(resname, "x")
    return NATURAL_RESIDUE_MAP.get(resname, "X")
            ...
            residues[chain].append(
                {
                    ...
                    "aa": residue_to_design_letter(record_name, resname),
                }
            )
            try:
...
def choose_predicted_peptide_chain(
    ...
    # Project rule confirmed by the senior researcher: the final chain is the
    # cyclic peptide. Dict insertion order preserves first appearance in PDB.
...
def ordered_ca_coordinates(selection: str) -> List[Tuple[float, float, float]]:
...
def evaluate_pdb(
    ...
    try:
        ...
        sequence_matches_design_exactly = decoded_design_seq == design_seq
        ...
        base.update(
            {
                ...
                "passes_joint_global_and_cyclic_peptide_lt_threshold": int(
...
def build_design_tables(
    ...
    for key, raw_rows in sorted(designs.items(), key=lambda item: item[0]):
        ...
        joint_valid = [
            ...
            if str(row.get("passes_joint_global_and_cyclic_peptide_lt_threshold", "")) == "1"
        ]
        ...
        if valid:
            ...
            exploratory = sorted(
                joint_valid or valid, key=representative_sort_best_rmsd
            )[0]
...
def main() -> None:
    ...
    best85_peptide_pass = [
        row for row in best85_rows
        if row.get("global_complex_ca_rmsd_status") == "ok"
        and str(row.get("passes_cyclic_peptide_ca_rmsd_lt_threshold", "")) == "1"
    ]
    best85_joint_pass = [
        row for row in best85_rows
        if row.get("global_complex_ca_rmsd_status") == "ok"
    ...
    write_csv(out_dir / "cyclic_peptide_ca_rmsd_best85_lt3.csv", best85_peptide_pass)
    write_csv(out_dir / "joint_global_and_cyclic_peptide_best85_lt3.csv", best85_joint_pass)
    write_csv(
    ...
    write_csv(
        out_dir / "cyclic_peptide_ca_rmsd_all_designs_confidence_representative_lt3.csv",
        confidence_peptide_pass,
    )
    write_csv(
        out_dir / "joint_global_and_cyclic_peptide_all_designs_confidence_representative_lt3.csv",
        confidence_joint_pass,
    )
    write_csv(
    ...
    write_csv(
        out_dir / "joint_global_and_cyclic_peptide_lt3_candidates_for_downstream.csv",
        downstream_candidates,
    )
    write_csv(
    ...
    sequence_ok = sum(
        str(row.get("decoded_design_seq_matches_design_naturalized", "")) == "1"
    ...
    combined_lines = [
        ...
        f"best85 joint pass: {len(best85_joint_pass)}/{len(best85_rows)}",
        "",
        f"all PDB global pass: {len(all_pdb_pass)}/{len(all_pdb_rows)}",
        f"all PDB cyclic-peptide pass: {len(all_pdb_peptide_pass)}/{len(all_pdb_rows)}",
        f"all PDB joint pass: {len(all_pdb_joint_pass)}/{len(all_pdb_rows)}",
        "",
        f"confidence representatives global pass: {len(confidence_pass)}/{len(confidence_designs)}",
        f"confidence representatives cyclic-peptide pass: {len(confidence_peptide_pass)}/{len(confidence_designs)}",
    ...
    print(
        f"best85 cyclic peptide: {len(best85_peptide_pass)}/{len(best85_rows)}; "
        f"joint: {len(best85_joint_pass)}/{len(best85_rows)}",
        flush=True,
    )
    print(
    ...
    print(
        f"all PDB cyclic peptide: {len(all_pdb_peptide_pass)}/{len(all_pdb_rows)}; "
        f"joint: {len(all_pdb_joint_pass)}/{len(all_pdb_rows)}",
        flush=True,
    )
    print(
