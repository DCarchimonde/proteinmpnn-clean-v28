...
If a previous run completed steps 1-7 and stopped at the WSL/PyRosetta step,
resume from step 8 without rebuilding the isolated workspace:

```powershell
powershell -ExecutionPolicy Bypass -File .\paper_clean_v28\structure_metrics\run_temperature05_best17_all.ps1 -StartStep 8
```

The controller converts drive-letter Windows paths directly to WSL mount
paths (for example, `E:\work` becomes `/mnt/e/work`) instead of passing raw
backslash paths through `wslpath`.
...
## Scope of the final table

The 17-row table contains complex-design, per-structure metrics only. It does
not duplicate dataset-level monomer classifier summaries such as monomer AUC,
F1, precision, or recall.

- Primary global/cyclic peptide RMSDs are carried from the complete 4108-PDB
  PyMOL run for the exact selected PDBs.
- Receptor-fit audit RMSDs and methylation-position RMSDs are recomputed in the
  isolated 17-structure workspace.
- HighFold confidence values and permeability predictions are matched to the
  exact 17 selected structures/sequences; they are not regenerated by this
  controller.
- Naturalized fixed-pose PyRosetta scores are recomputed for all 17 rows.
- Monomer structural metrics require a separate monomer PDB input set and are
  not produced by this complex-only controller.
